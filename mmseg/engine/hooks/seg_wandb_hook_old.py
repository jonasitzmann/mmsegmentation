...
class MMSegWandbHook(LoggerHook):
    ...
    def before_run(self, runner: Runner):
        ...
        if self.log_evaluation or self.log_checkpoint_metadata:
            if runner.val_evaluator is None:
                ...
                self.eval_interval = runner.val_interval
    ...
    def after_train_iter(self, runner, batch_idx, data_batch, outputs, *args, **kwargs):
        ...
        #     An ugly patch. The iter-based eval hook will call the
            ...
            return super().after_train_iter(runner, batch_idx=batch_idx, data_batch=data_batch, outputs=outputs, *args, **kwargs)
        else:
            super().after_train_iter(runner, batch_idx=batch_idx, data_batch=data_batch, outputs=outputs, *args, **kwargs)
        self._after_train_iter(runner, batch_idx=batch_idx, data_batch=data_batch, outputs=outputs)

    # for the reason of this double-layered structure, refer to
    # https://github.com/open-mmlab/mmdetection/issues/8145#issuecomment-1345343076
    def after_val_iter(self, runner, batch_idx, data_batch, outputs, *args, **kwargs):
        super().after_val_iter(runner, batch_idx=batch_idx, data_batch=data_batch, outputs=outputs, *args, **kwargs)
        self._after_train_iter(runner, batch_idx=batch_idx, data_batch=data_batch, outputs=outputs)

    @master_only
    def _after_train_iter(self, runner: Runner, batch_idx, data_batch, outputs):
        ...
        if self.log_evaluation:
            ...
            results = runner.val_evaluator.evalutate(1)
            # Initialize evaluation table
    ...
    def _log_ckpt_as_artifact(self, model_path, aliases, metadata=None):
        ...
        """
        model_artifact = wandb.Artifact(f'run_{self.wandb.id}_model', type='model', metadata=metadata)
        model_artifact.add_file(model_path)
        self.wandb.log_artifact(model_artifact, aliases=aliases)

    def _get_evalresults(self):
